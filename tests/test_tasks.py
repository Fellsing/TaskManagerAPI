...
async def auth_headers(ac: AsyncClient):
    user_response = await ac.post(
        "/auth/signup",
        json={
            "username": "Fell",
            "email": "fellsing13@gmail.com",
            "password": "Password123!",
        },
    )

    user_login_response = await ac.post(
        "/auth/signin",
        data={"username": "fellsing13@gmail.com", "password": "Password123!"},
    )
...
async def add_task(ac: AsyncClient, auth_headers):
    adding_task = await ac.post(
        "/task/add",
        json={
            "title": "strr",
            "description": "strr descr",
            "deadline": "2036-04-10T15:00:00",
        },
...
async def test_create_task(ac: AsyncClient):
    ...
    assert (
        response.json()["owner_id"] == user_id
    )  # Данная проверка и в целом декодирование в тесте излишне, применено только в целях демонстрации
...
async def test_delete_task(ac: AsyncClient, auth_headers):
    ...
    task_id = adding_task.json()["id"]
    res = await ac.delete(f"/task/delete/{task_id}", headers=auth_headers["headers"])
...
async def test_patch_update_task(ac: AsyncClient, auth_headers):
    ...
    response = await ac.patch(
        ...
        json={
            ...
            "description": "strr super insane descr",
            "deadline": "2056-04-10T15:00:00",
        },
    )
    assert response.status_code == 200
...
async def test_get_task_by_id(ac: AsyncClient, auth_headers):
    ...
    response = await ac.get(f"/task/me/{task_id}", headers=auth_headers["headers"])
    assert response.status_code == 200
...
async def test_access_denied(ac: AsyncClient, auth_headers, add_task):
    ...
    user_login_response = await ac.post(
        "/auth/signin",
        data={"username": "hacker@gmail.com", "password": "Password123!"},
    )
...
async def test_refresh(ac: AsyncClient, auth_headers):
    old_access_token = auth_headers["access_token"]
    ...
    duplicate_refresh_attempt = await ac.post(
        "/auth/refresh", json={"refresh_token": old_refresh_token}
    )
    assert duplicate_refresh_attempt.status_code == 401
